...
static bool8 LoadBattlerSpriteGfx(u8 battler)
{
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            ...
            // DecompressTrainerBackPic(gSaveBlock2Ptr->playerForm, battler);
            DecompressTrainerBackPalette(gSaveBlock2Ptr->playerForm + TRAINER_BACK_PIC_PROTAG_MALE_1, battler);
...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(gSaveBlock2Ptr->playerForm + TRAINER_BACK_PIC_PROTAG_MALE_1, GetBattlerPosition(B_POSITION_PLAYER_LEFT));
